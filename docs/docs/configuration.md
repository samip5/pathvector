...
| direct  | bool | false |  | Are we an direct session? |
...
| filter-never-via-route-servers | bool | false |  | Should routes containing an ASN reported in PeeringDB to never be reachable via route servers be filtered? |
